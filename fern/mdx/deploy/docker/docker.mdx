...
Core services, with the exception of Supabase can all be spun up in docker with one script.
...
In order to deploy all the core services an example script is provided in `libs\.docker\run.all.sh`.  You can modify this script to ommit any services by commenting out or deleting lines.

To run all of these services in docker, cd into the `libs/.docker` folder, run the script `run.all.sh`:
...
### Optionally omit any services if you have them deployed elsewhere
