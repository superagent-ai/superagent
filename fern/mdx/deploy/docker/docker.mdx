...
5. **Langfuse** - Open source telemetry for LLMs
...
Example configuration for the core components of Superagent is located in the `./libs/.docker/.env.example` file.
...
Dependencies outside of the core components have their own `.env` files colocated with their docker-compose files, either because they are optional/swappable (e.g vector stores) or have a lot of configuration (e.g Supabase).

### Docker networking and hosts file

As there are a lot of services that require environment variables set with their URLs, that will need to be accessible both internally in the docker network and locally from your machine, it is neccessary to set the various docker service names in your operating systems `hosts` file. This ensures services can be accessed in the browser with the same url as is used internally inside docker between the services.
...
Refer to our guide [Supabase setup](https://docs.superagent.sh/overview/deployments/render). The guide explains how to run Supabase with docker compose using our provided example, or how to setup Supabase cloud if preferred.

## 2. Deploy Core components

Core components, with the exception of Supabase can all be spun up in docker with one script.

The main docker compose file is located in `libs\.docker\docker-compose.yml` and contains only the Superagent API. Other core components are provided as sub-compose files that can be appended to the `docker compose up` command to deploy them all at once.

In order to deploy all the core components an example script is provided in `libs\.docker\run.all.sh`.  You can modify this script to ommit any components by commenting out or deleting lines.

To run all of these components in docker, cd into the `libs/.docker` folder, run the script `run.all.sh`:
...
- `superagent-api` - Superagent backend api
...
- `pgdb` - Postgres DB for the Superagent backend api
- `pgadmin` - User interface for the Postgres DB

### Optionally omit any components if you have them deployed elsewhere

The `libs/.docker/run.all.sh` script looks like this and includes all the core dependencies. You can omit any part that you may wish to deploy elsewhere. For example, if you wanted to use a cloud hosted database for the superagent api, then you can comment/delete the lines for `-f superagent/db/docker-compose.pgdb.yml \` and `-f superagent/db/docker-compose.pgadmin.yml \` then modify the `libs/.docker/.env` file with the configuration for your own POSTGRES db.
...
Refer to our guide [Langfuse setup](https://docs.superagent.sh/overview/deployments/render). The guide explains how to run a local version of Langfuse with docker compose using our provided example, or how to setup Langfuse cloud if preferred.

## Deploy a vector store
...
- **Weaviate** - [Weaviate Setup Guide](https://superagent.sh)
- More guides coming soon

## logs

View the logs for any of the services using the `container-name` provided in each `docker-compose.yml` file. Add `-f` to the command to follow the logs, and `ctrl+c` to exit the logs, if following.
