...
- `options` (optional) – Options object
  ...
  - `file` – File or Blob object to redact (e.g., PDF document)
  - `format` – Output format: `"json"` (default) returns JSON with redacted text, `"pdf"` returns a redacted PDF file
...
interface RedactOptions {
  ...
  entities?: string[];      // Custom entity types to redact
  file?: File | Blob;       // File to redact (e.g., PDF)
  format?: "json" | "pdf";  // Output format
}

interface RedactResult {
  redacted: string;         // Text with sensitive data redacted (empty when format="pdf")
  reasoning: string;        // Explanation of what was redacted
  usage?: GuardUsage;       // Token usage statistics
  raw: RedactionResponse;   // Full API response (empty when format="pdf")
  pdf?: Blob;               // PDF Blob when format="pdf"
  redacted_pdf?: string;    // Base64 PDF data URL when file provided with JSON response
}
```
...
const result = await client.redact(
  "Extract and redact text from this document",
  {
    file: pdfBlob,
