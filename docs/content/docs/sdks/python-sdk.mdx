...
- `entities` (optional) – List of natural language descriptions of PII entities to redact. Examples: `["credit card numbers", "email addresses", "phone numbers"]`
- `file` (optional) – File object to redact (e.g., PDF document). Pass an open file handle in binary mode.
- `format` (optional) – Output format: `"json"` (default) returns JSON with redacted text, `"pdf"` returns redacted PDF bytes.
...
@dataclass
class RedactResult:
    redacted: str                # Text with sensitive data redacted (empty when format="pdf")
    reasoning: str               # Explanation of what was redacted
    raw: dict                    # Full API response (empty when format="pdf")
    usage: Optional[GuardUsage]  # Token usage statistics
    pdf: Optional[bytes]         # PDF bytes when format="pdf"
    redacted_pdf: Optional[str]  # Base64 PDF data URL when file provided with JSON response
```
...
async def main() -> None:
    async with create_client(api_key="sk-...") as client:
        ...
        with open("sensitive-document.pdf", "rb") as pdf_file:
            result = await client.redact(
                text="Extract and redact text from this document",
                file=pdf_file,
...
**How it works:**
- When a `file` is provided, the SDK uses multipart/form-data encoding
- The `text` parameter provides context for the redaction
