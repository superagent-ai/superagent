...
<Callout>Sign up at [app.superagent.sh](https://app.superagent.sh) to obtain an API Key</Callout>
...
**Syntax:**
```bash title="Terminal"
superagent analyze [options] <file>
```

**Arguments:**
- `<file>` - The file or path to analyze (can be omitted if using stdin)

**Options:**
- `--file <path>` - Path to file to analyze (PDF, etc.)
- `--help` - Show help message
...
superagent analyze --file document.pdf

# Analyze from stdin (JSON format)
echo '{"file": "{{ANALYZE_EXAMPLE_INPUT}}"}' | superagent analyze

# Show help
superagent analyze --help
```
...
**How it works:**
- The `--file` flag accepts a path to a file (PDF, etc.)
...
{
  "decision": "block",
  "reason": "=Ã¯Â¿Â½ Superagent Guard blocked this prompt: User requests destructive action. Violations: malicious_action. CWE: CWE-77.",
  "hookSpecificOutput": {
...
**Error:**
```
Ã¯Â¿Â½ Guard check failed: This operation was aborted
```
