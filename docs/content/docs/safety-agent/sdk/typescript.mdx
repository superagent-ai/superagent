---
title: TypeScript
description: TypeScript SDK for content safety - guard, redact, and scan methods
---

# TypeScript SDK

The `@superagent-ai/safety-agent` package provides three core methods: `guard()` for detecting threats, `redact()` for removing sensitive data, and `scan()` for analyzing repositories for AI agent-targeted attacks.
