...
| `prompt` | `str` | No | Security prompt | Custom scanning prompt |
...
| `reasoning` | `str` | Summary explanation of findings |
| `findings` | `list[ScanFinding]` | List of security findings |
...
if result.summary.critical > 0 or result.summary.high > 0:
    print("Critical/high severity issues found!")
