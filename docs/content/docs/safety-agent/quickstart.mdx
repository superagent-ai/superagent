...
3. Set the `SUPERGENT_API_KEY` environment variable or pass it to `createClient()`

<Callout type="info">
  The Superagent guard model is used by default and requires no API keys. The `SUPERGENT_API_KEY` is optional and only used for usage tracking.
</Callout>
...
No API keys required for the default guard model. Optionally set `SUPERGENT_API_KEY` for usage tracking:

```bash
export SUPERGENT_API_KEY=your-key
```
