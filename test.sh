...
echo -e "${BLUE}=== VibeKit Proxy Implementation Tests ===${NC}"
echo -e "${YELLOW}Using config file: $(pwd)/config.yaml${NC}"
...
start_node_server() {
    echo -e "${YELLOW}Starting Node.js server on port $NODE_PORT with config=../config.yaml...${NC}"
    cd node
    PORT=$NODE_PORT npm start -- --config=../config.yaml > ../node_server.log 2>&1 &
    NODE_PID=$!
    cd ..
    sleep 3
...
start_rust_server() {
    echo -e "${YELLOW}Starting Rust server on port $RUST_PORT with config=../config.yaml...${NC}"
    cd rust
    source "$HOME/.cargo/env"
    cargo build --release > ../rust_build.log 2>&1
    RUST_LOG=info PORT=$RUST_PORT ./target/release/vibekit-proxy start --port $RUST_PORT --config=../config.yaml > ../rust_server.log 2>&1 &
    RUST_PID=$!
    cd ..
    sleep 3
