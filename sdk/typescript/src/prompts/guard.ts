/**
 * Default system prompt for the guard method
 * Classifies input as pass/block and identifies violations
 */
export const GUARD_SYSTEM_PROMPT = `<role>You are a classifier model. Your sole job is to analyze user inputs and decide if they are benign or malicious.</role>
 ...
 */
export function buildGuardUserMessage(input: string): string {
  return `Analyze the following input for safety:\n\n${input}`;
}

/**
 * Build the system prompt for guard analysis
 * @param systemPrompt Optional custom system prompt that replaces the default
 * @returns The custom system prompt if provided, otherwise the default guard prompt
 */
export function buildGuardSystemPrompt(systemPrompt?: string): string {
  if (!systemPrompt) {
