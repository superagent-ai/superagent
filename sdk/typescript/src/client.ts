  ...
  async guard(options: GuardOptions): Promise<GuardResponse> {
    ...
    try {
      ...
      if (chunkSize === 0 || text.length <= chunkSize) {
        ...
        const segmentStart = Date.now();
        const result = await this.guardSingleText(text, systemPrompt, model, fallbackModel).catch(
          (error) => {
            markGuardHooked(error);
            this.emitHook(
              hooks?.onError,
      ...
      const results = await Promise.all(
        chunks.map(async (chunk, index) => {
          const segmentStart = Date.now();
          try {
            const result = await this.guardSingleText(chunk, systemPrompt, model, fallbackModel);
            this.emitHook(
              hooks?.onSegment,
              buildGuardSegmentEvent({
                kind: "chunk",
