...
from app.workflows.base import WorkflowBase, WorkflowStep
...
async def invoke(
    ...
    for idx, step in enumerate(workflow_data.steps):
        ...
        workflow_step = WorkflowStep(
            ...
            callbacks=[
                CostCalcCallback(model=llm_model),
            ],
        )

        # logging_handlers = get_logging_handlers(
        ...
        # workflow_step.callbacks.extend(logging_handlers)

        if enable_streaming:
            workflow_step.callbacks.append(CustomAsyncIteratorCallbackHandler())
    ...
    def track_invocation(output):
        for index, workflow_step in enumerate(workflow_steps):
            ...
            for callback in workflow_step.callbacks:
                if isinstance(callback, CostCalcCallback):
                    cost_callback = callback
                    break

            if not cost_callback:
                logger.warning(
                    f"Cost callback not found for step {workflow_step.agent_data.name}"
                )
                continue
                    ...
                    async for token in streaming_callback.aiter():
                        if not output_schema:
                            async for val in stream_dict_keys(
                                {
                                    "id": workflow_step.agent_data.name,
                                    "data": token,
                                }
                            ):
                                yield val
                            ...
                            async for val in stream_dict_keys(
                                {
                                    "event": "function_call",
                                    "data": {
                                        "step_name": workflow_step.agent_data.name,
                                        "function": function,
    ...
    output = await workflow.arun(input=input)
